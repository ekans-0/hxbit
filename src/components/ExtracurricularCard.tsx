...
export function ExtracurricularCard({ extracurricular, tasks, onDelete }: ExtracurricularCardProps) {
  ...
  return (
    <div 
      className="bg-gray-900 border border-gray-800 rounded-xl p-6 hover:border-blue-500/50 transition-all duration-300 hover:transform hover:scale-105 hover:shadow-xl hover:shadow-blue-500/10 relative group"
      style={{
    ...
    >
      ...
      {onDelete && (
        <button
          ...
          className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-gray-400 hover:text-red-500 p-1 rounded-full hover:bg-red-50 dark:hover:bg-red-900/20"
        >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">{extracurricular.description}</p>
          </div>
        </div>
        <div className="text-right">
          ...
          <p className="text-xs text-gray-500">{xpToNextLevel} XP to next</p>
        </div>
      </div>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-medium text-gray-300">Level Progress</span>
          <span className="text-xs text-gray-400">{currentLevelXp} / 100 XP</span>
        </div>
      ...
      {tasks.length > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-800">
          <p className="text-xs text-gray-500 mb-2">Recent Task:</p>
          <div className="flex items-center">
            <div 
            ...
            ></div>
            <p className="text-sm text-gray-300 truncate">{tasks[0].title}</p>
          </div>
        </div>
      )}
    </div>
